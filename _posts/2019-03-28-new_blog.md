---
layout: post
title: "如何在 github 上搭建自己的Blog ?"
subtitle: "New blog , new start."
author: "beforenight"
...
> 很久没有动笔在 Github 上写东西了，时间在流，我也在走；这里不应该是一片被遗忘的绿洲。写写记记，换个主题；感谢 Hux 的主题，感谢开源。

回到正题：如何在 github 上搭建自己的Blog ?

首先，你要有个 github 账户，当然，如果没有点击这里 [Github Home Page](https://github.com/) 就可以去注册。
GitHub Pages 是免费提供给用户的服务，在 Github 上，是这样介绍 GitHub Pages 的：
> GitHub Pagesis designed to host your personal, organization, or project pages from a GitHub repository.

然后，有了账号就可以愉快的在本地搭建一套静态 Blog 服务，在这里，我们借助于 [Jekyll](https://www.jekyll.com.cn/) ,Jekyll 可以帮助我们 —— "将纯文本转化为静态网站和博客"。这里有一份完整的教程可以快速本地建站，发布到 Github 上。

> Jekyll 究竟是什么？
> Jekyll 是一个简单的博客形态的静态站点生产机器。它有一个模版目录，其中包含原始文本格式的文档，> 通过 Markdown （或者 Textile） 以及 Liquid 转化成一个完整的可发布的静态网站，你可以发布在> 任何你喜爱的服务器上。Jekyll 也可以运行在 GitHub Page 上，也就是说，你可以使用 GitHub 的> 服务来搭建你的项目页面、博客或者网站，而且是完全免费的。
...
服务启动后，在浏览器输入：http://localhost:4000 或 http://127.0.0.1:4000/ 即可访问bolg.

预览效果如下：
![Beforenight Blog](https://wx3.sinaimg.cn/mw1024/5f88c6f8ly1g1imb63tzfj21pe0p4u0x.jpg "Beforenight")


当然，默认的主题肯定不是上面截图这样子，更多的玩法需要参考文档进行修改对应的主题和配置文件；然后学习如何发布新的文章到 Blog 上；在然后，需要在 Github 上创建 <username>.github.io 的项目，将本地运行的 _site 目录下的全部文件发不到这个项目中就可以通过 http://<username>.github.io 访问。

当然熟悉了初始化建站流程，我们就可以参考开源平台上其他的作者分享的Blog进行修改，也能过快速建站；这里还有一份文档说明，希望对你有帮助——
